import { BuyRequest, Platform, prisma } from '@repo/prisma'
import { rabbitmqService } from './rabbitmq'
import { lisSkinsParser, BuyRequestQuery } from '../parsers/lis-skins'
import { env } from '../env'
...
class ParserScheduler {
  ...
  private async processBuyRequest(buyRequest: BuyRequest): Promise<void> {
    ...
    for (const item of foundItems) {
      ...
      logger.info({ buyRequestId: buyRequest.id, itemName: item.name, price: item.price }, `Found matching item: ${item.name} - ${item.price}₽`)
    }
  }
}
