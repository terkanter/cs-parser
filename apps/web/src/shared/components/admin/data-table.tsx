import { BulkActionsToolbar, BulkActionsToolbarChildren } from "@/shared/components/admin/bulk-actions-toolbar";
import { ColumnsSelector, ColumnsSelectorItem } from "@/shared/components/admin/columns-button";
...
import { Children, type ReactNode, createElement, isValidElement, useCallback } from "react";
import { useNavigate } from "react-router";

const defaultBulkActionButtons = <BulkActionsToolbarChildren />;

export function DataTable<RecordType extends RaRecord = RaRecord>(props: DataTableProps<RecordType>) {
  const {
    children,
    className,
    rowClassName,
  ...
  return (
    <DataTableBase<RecordType> hasBulkActions={hasBulkActions} loading={null} empty={<DataTableEmpty />} {...rest}>
...
const DataTableRow = ({
  ...
  return (
    <TableRow key={record.id} onClick={handleClick} className={cn(rowClick !== false && "cursor-pointer", className)}>
      {hasBulkActions ? (
...
function DataTableHeadCell<RecordType extends RaRecord<Identifier> = RaRecord<Identifier>>(
  ...
  const isColumnHidden = hiddenColumns.includes(source as string);
  if (isColumnHidden) return null;

  const nextSortOrder = sort && sort.field === source ? oppositeOrder[sort.order] : (sortByOrder ?? "ASC");
  ...
  const sortLabel = translate("ra.sort.sort_by", {
    field: fieldLabel,
    field_lower_first:
      typeof fieldLabel === "string" ? fieldLabel.charAt(0).toLowerCase() + fieldLabel.slice(1) : undefined,
...
function DataTableCell<RecordType extends RaRecord<Identifier> = RaRecord<Identifier>>(
  ...
  if (!render && !field && !children && !source) {
    throw new Error("DataTableColumn: Missing at least one of the following props: render, field, children, or source");
  }

  return (
    <TableCell className={cn("py-1", className, cellClassName, record && conditionalClassName?.(record))}>
      {children ??
        (render ? record && render(record) : field ? createElement(field, { source }) : get(record, source!))}
    </TableCell>
  );
}

export interface DataTableColumnProps<RecordType extends RaRecord<Identifier> = RaRecord<Identifier>> {
  className?: string;
...
export function DataTableNumberColumn<RecordType extends RaRecord<Identifier> = RaRecord<Identifier>>(
...
export interface DataTableNumberColumnProps<RecordType extends RaRecord<Identifier> = RaRecord<Identifier>>
