import { AddSavedQueryDialog, RemoveSavedQueryDialog } from "@/shared/components/admin/saved-queries";
...
import { type FilterElementProps, useFilterContext } from "@/shared/hooks/filter-context";
import { type SavedQuery, extractValidSavedQueries, useSavedQueries } from "@/shared/hooks/saved-queries";
import { cn } from "@/shared/lib/utils";
import get from "lodash/get";
import isEqual from "lodash/isEqual";
import { Bookmark, BookmarkMinus, BookmarkPlus, Check, Filter, MinusCircle, X } from "lucide-react";
import queryString from "query-string";
import { FieldTitle, FilterLiveForm, useListContext, useResourceContext, useTranslate } from "ra-core";
...
export const FilterFormBase = (props: FilterFormBaseProps) => {
  ...
  const handleHide = useCallback(
    (event: React.MouseEvent<HTMLElement>) => hideFilter(event.currentTarget.dataset.key as string),
    [hideFilter],
  );
...
export const FilterButton = (props: FilterButtonProps) => {
  const { filters: filtersProp, className, disableSaveQuery, size, variant = "outline", ...rest } = props;
    ...
    <div className={cn("inline-block", className)} {...rest}>
      <DropdownMenu open={open} onOpenChange={setOpen}>
        <DropdownMenuTrigger asChild>
          <Button type="button" className="add-filter" variant={variant} size={size} aria-haspopup="true">
            <Filter className="h-4 w-4" />
            {translate("ra.action.add_filter")}
          </Button>
        </DropdownMenuTrigger>
        <DropdownMenuContent align="start" className="w-56">
          {allTogglableFilters.map((filterElement: React.ReactElement<FilterElementProps>, index: number) => (
          ...
          {validSavedQueries.map((savedQuery: SavedQuery, index: number) =>
            isEqual(savedQuery.value, {
              ...
              <DropdownMenuItem onClick={showRemoveSavedQueryDialog} key={savedQuery.label}>
      ...
      {!disableSaveQuery && (
        <>
          ...
          <RemoveSavedQueryDialog open={removeSavedQueryDialogOpen} onClose={hideRemoveSavedQueryDialog} />
        </>
      )}
    </div>
  );
...
export const FilterButtonMenuItem = React.forwardRef<HTMLDivElement, FilterButtonMenuItemProps>((props, ref) => {
  ...
  return (
    ...
    >
      <div className="flex items-center justify-center w-4 h-4 mr-2">{displayed && <Check className="h-3 w-3" />}</div>
      <div>
