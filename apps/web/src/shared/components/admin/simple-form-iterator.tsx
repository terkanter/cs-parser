import { Confirm } from "@/shared/components/admin/confirm";
import { IconButtonWithTooltip } from "@/shared/components/admin/icon-button-with-tooltip";
...
import { Children, type ReactElement, type ReactNode, useCallback, useMemo, useRef, useState } from "react";
  ...
  (props: SimpleFormIteratorItemProps, ref: React.ForwardedRef<HTMLLIElement>) => {
    ...
    return (
      <SimpleFormIteratorItemContext.Provider value={context}>
        ...
        >
          {label != null && label !== false && <p className="text-sm text-muted-foreground mb-2">{label}</p>}
          <SourceContextProvider value={sourceContext}>
            <RecordContextProvider value={record}>
              <div className={cn("flex flex-1 gap-2", inline ? "flex-col sm:flex-row gap-2" : "flex-col")}>
                {children}
              </div>
            </RecordContextProvider>
          </SourceContextProvider>
...
export const ReOrderButtons = ({ className }: { className?: string }) => {
  ...
  return (
    <span className={cn("button-reorder", `button-reorder-${source}-${index}`, className)}>
      <IconButtonWithTooltip label="ra.action.move_up" onClick={() => reOrder(index - 1)} disabled={index <= 0}>
