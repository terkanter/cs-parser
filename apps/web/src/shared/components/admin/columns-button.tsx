import { FieldToggle } from "@/shared/components/admin/field-toggle";
...
export const ColumnsSelectorItem = <RecordType extends RaRecord<Identifier> = RaRecord<Identifier>>({
  source,
  label,
...
export interface ColumnsSelectorItemProps<RecordType extends RaRecord<Identifier> = RaRecord<Identifier>> {
  className?: string;
...
const fieldLabelMatchesFilter = (fieldLabel: string, columnFilter?: string) =>
  columnFilter ? diacritic.clean(fieldLabel).toLowerCase().includes(diacritic.clean(columnFilter).toLowerCase()) : true;
