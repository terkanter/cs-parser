...
import { FieldTitle, type InputProps, RecordContextProvider, shallowEqual, useInput, useTranslate } from "ra-core";
...
export const FileInput = (props: FileInputProps) => {
  ...
  const childrenElement =
    children && isValidElement(Children.only(children)) ? (Children.only(children) as ReactElement) : undefined;
  ...
  return (
    <FormField id={id} name={name} className={cn("w-full", className)} {...rest}>
      <FormLabel htmlFor={id} className={disabled || readOnly ? "cursor-default" : "cursor-pointer"}>
      ...
      <div
        {...getRootProps({
          className: cn(
            ...
            disabled || readOnly ? "bg-muted cursor-not-allowed" : "bg-muted text-muted-foreground cursor-pointer",
          ),
        })}
      >
        <input
        ...
        <div className="previews flex flex-col gap-1">
          {
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            files.map((file: any) => (
              <FileInputPreview key={file.title} file={file} onRemove={onRemove(file)} removeIcon={removeIcon}>
