...
export const AutocompleteArrayInput = (
  ...
  const { allChoices = [], source, resource, isFromReference, setFilters } = useChoicesContext(props);
  ...
  const availableChoices = allChoices.filter((choice) => !field.value.includes(getChoiceValue(choice)));
  const selectedChoices = allChoices.filter((choice) => field.value.includes(getChoiceValue(choice)));
  ...
  return (
    <FormField className={props.className} id={id} name={field.name}>
      {props.label !== false && (
        <FormLabel>
          <FieldTitle label={props.label} source={props.source ?? source} resource={resource} isRequired={isRequired} />
        </FormLabel>
      )}
      <FormControl>
        <Command onKeyDown={handleKeyDown} shouldFilter={!isFromReference} className="overflow-visible bg-transparent">
