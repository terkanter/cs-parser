...
export const FieldToggle = (props: FieldToggleProps) => {
  ...
  const handleDragEnd = (event: React.DragEvent) => {
    ...
    const elementFromPoint =
      x.current != null && y.current != null ? document.elementFromPoint(x.current, y.current) : null;
    ...
    if (dropItem && list && dropItem.closest("ul") === list) {
      if (onMove) onMove(selectedItem.dataset.index as string, dropIndex.current);
    } else {
  ...
  return (
    ...
    >
      <label htmlFor={`switch_${index}`} className="flex items-center gap-2 cursor-pointer">
        <Switch id={`switch_${index}`} checked={selected} onCheckedChange={onToggle} name={`${index}`} />
